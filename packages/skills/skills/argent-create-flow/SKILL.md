...
| `type`       | `- type: { into: email, text: "a@b.com" }`, `- type: { into: email, text: "a@b.com", clear: true }`, `- type: { into: search, clear: true }`                         | focus a field, type, then press Enter to submit + dismiss the keyboard; `clear: true` empties the field first — without it the text is appended to whatever is already there |
...
`type` **appends**. Set `clear: true` whenever the field may already hold a value — a remembered login, a restored draft, a fragment re-run without a `launch`, a retry after a failed step: `type: { into: email, text: "new@example.com", clear: true }`. Without it the step types on top of the old value and the flow fails later at an `assert`, pointing at the wrong place. `clear` works on iOS, Android (API 30+) and Chromium; it is rejected on Vega, on TV targets, and on older Android. A `clear` that does not empty the field fails the step immediately rather than appending.

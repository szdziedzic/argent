...
A directive that fails hard-stops the flow, so a mis-targeted assert costs you every later step. Where the contents are invisible, assert the _consequence_ instead (the filtered list, the enabled submit button, the cleared error).

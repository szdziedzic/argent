...
`type` never **replaces**. The old value survives, and the new text lands wherever the focusing tap put the caret — which is not always the end: the tap goes to the middle of the field, so on Android a value long enough to reach that point gets the new text spliced _into_ it (a field holding forty `A`s, typed with `ZZZ`, ends up `AAA…ZZZ…AAA`), while on iOS the caret stays put and the text appends. Set `clear: true` whenever the field may already hold a value — a remembered login, a restored draft, a fragment re-run without a `launch`, a retry after a failed step: `type: { into: email, text: "new@example.com", clear: true }`. Without it the flow fails later at an `assert`, pointing at the wrong place.

`clear` works on iOS, Android and Chromium; it is rejected — loudly, before anything is typed — on TV targets, whose input transport cannot send the select-all chord. (On Vega the whole `type` directive is rejected, `clear` or not — see _TV targets_.) A clear-only step sends no Enter; add `submit: true` when the empty value itself has to be committed, e.g. to reset a live-filtered list.

The step will not clear blind: if the tree reports focus landing somewhere other than the field you named, the step **fails** instead of emptying whatever else holds it. It does not read the field back afterwards, though. If a flow must prove the new value took, follow it with an `assert` on the field's text — but check the platform first, because a text field's contents reach the flow tree unevenly: **Android** exposes them next to the hint (so use `contains`, not `equals`, which would have to match hint and value together); **iOS** never exposes them at all, so the assert reads the field's label and fails on a perfectly good clear; **Chromium** exposes them only for an `<input>` carrying no `aria-label` and no `placeholder`, and otherwise reads that label instead — which passes whether or not the clear happened. Where the contents are invisible, assert the _consequence_ instead (the filtered list, the enabled submit button, the cleared error).
...
<<<<<<< HEAD
   - `tool: keyboard` typing into a field → `type: { into: "<field>", text: "…" }`, folding in the `tap` that focused the field. **Carry `clear` across**: a recorded `tool: keyboard` step with `args: { clear: true }` becomes `type: { into: "<field>", text: "…", clear: true }`, and one with `clear` but no `text` becomes `type: { into: "<field>", clear: true }`. Dropping it turns a replace back into an append, which is a data bug the cleaned flow will only reveal at a later `assert`. A submit, by contrast, is recorded as its **own** `tool: keyboard { key: enter }` step (the tool takes text or key, never both) — drop that one, since `type` already presses Enter, or keep it and set `submit: false`. Leaving both submits twice.
=======
...
>>>>>>> d48716a6 (fix(flow): make `clear` atomic with the text, and never clear blind)
